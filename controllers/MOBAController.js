exports.createGame = async(req, res) => {
	try {
		let game = await MOBA.findOne({name: req.body.name});
		if (game)
		{
...
exports.getGame = async(req, res) => {
	try {
		const game = await MOBA.findById(req.params.id);
		return res.status(200).json({
			status: 'get success',
			game
		});
...
exports.updateGame = async(req, res) => {
	...
	try {
		const game = await MOBA.findByIdAndUpdate(req.params.id,
			{
				Game: req.body.Game,
				Year: req.body.Year,
...
exports.deleteGame = async(req, res) => {
	try {
		const game = await MOBA.findByIdAndDelete(req.params.id);
		return res.status(200).json({
